...
def ensure_string(value):
    """Ensure value is a string, handling Django's translation proxies"""
    return force_str(value) if value else ""
...
class BrevoAPIBackend(BaseEmailBackend):
    ...
    def _send_email_via_brevo(self, message: EmailMessage):
        ...
        data = {
            "sender": {"email": ensure_string(message.from_email)},
            "to": [{"email": ensure_string(recipient)} for recipient in message.to],
            "subject": ensure_string(message.subject),
            "textContent": ensure_string(message.body)  # Plain text version
        }
        ...
        if html_content:
            data['htmlContent'] = ensure_string(html_content)
